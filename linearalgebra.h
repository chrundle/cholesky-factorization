...
/* ---------------------- subscalar_div --------------------- */
/*  Given an array, its length, a starting index, and a
    scalar value this function divides each value in the 
    array by the scalar value and stores the resulting value
    in the array.
    ...
    Input variables:
        x     : pointer to array whose components are to be
                 divided by r and stored in second array, y.
        r     : scalar used in division.
        index : starting index of subarray of x in which
                 the values are to be divided by r.
        length: number of entries in x.

    Features: This implementation has time complexity 
    O(length - index) and requires O(1) additional memory.    */

void subscalar_div (double * x, double r, int index, int length) {
    int i, length5;
...
/*  Given two arrays, the length of the second array, a scalar 
    ...
    Input variables:
        x     : pointer to array whose components are to be
                 multiplied by r then subtracted from the
                 components original array. The elements of 
                 the array should be the columns of the 
                 matrix A.
        r     : scalar used in multiplication.
        length: number of entries in the subarray of x.
